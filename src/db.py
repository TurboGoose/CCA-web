...
def create_tables() -> None:
    ...
    cur.execute(
        "CREATE TABLE IF NOT EXISTS datasets(name TEXT PRIMARY KEY"
...
def get_datasets() -> list[str]:
    con = sqlite3.connect(DATABASE_FILE)
    cur = con.cursor()
    cur.execute("SELECT name FROM datasets;")
...
def rename_dataset(old_dataset_name: str, new_dataset_name: str):
    ...
    cur = con.cursor()
    cur.execute(
        f"UPDATE datasets SET name='{new_dataset_name}' WHERE name='{old_dataset_name}';"
    )
    con.commit()
    con.close()
...
def save_label_for_dataset(label: str, dataset: str) -> None:
    ...
    cur = con.cursor()
    cur.execute(
        f"INSERT INTO labels (name, dataset_name) VALUES ('{label}', '{dataset}');"
    )
    con.commit()
    con.close()
...
def rename_label_for_dataset(
    dataset: str, old_label_name: str, new_label_name: str
) -> None:
    con = sqlite3.connect(DATABASE_FILE)
    cur = con.cursor()
    cur.execute(
        f"UPDATE labels SET name='{new_label_name}' "
        f"WHERE dataset_name='{dataset}' AND name='{old_label_name}';"
    )
    con.commit()
    con.close()
...
def delete_label_for_dataset(label: str, dataset: str) -> None:
    ...
    cur = con.cursor()
    cur.execute(
        f"DELETE FROM labels WHERE dataset_name='{dataset}' AND name='{label}';"
    )
    con.commit()
    con.close()
